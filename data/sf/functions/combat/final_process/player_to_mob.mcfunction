...
tag @s add Attacker
#体力/防御力情報をNBTから抽出
execute as @e[tag=Target] store result score @s MobDEF_CB run data get entity @s ArmorItems[3].tag.DEF
execute as @e[tag=Target] store result score @s MobHealth_CB run data get entity @s ArmorItems[3].tag.Health
#レベルによるATK_CBボーナス
...
#体力をNBTに格納
execute as @e[tag=Target] store result entity @s ArmorItems[3].tag.Health int 1.0 run scoreboard players get @s MobHealth_CB
#ダメージログ
...
#リセット
scoreboard players reset @e[tag=Target] MobHealth_CB
